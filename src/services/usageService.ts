...
export const getUserQuota = async (): Promise<UserQuota> => {
  ...
  try {
    ...
    if (userData) {
      ...
      // 管理员用户可以手动编辑使用量，优先使用手动设置的值
      const isAdmin = user.email === 'max.z.software@gmail.com';
      let usedMinutes = realUsedMinutes;
      
      if (isAdmin && user.usedMinutes !== undefined) {
        // 管理员用户使用手动设置的使用量
        usedMinutes = user.usedMinutes;
        console.log('🔧 管理员使用手动设置的使用量:', usedMinutes);
      }
      ...
      return {
        ...
        trialUsed: userType === 'trial' ? usedMinutes : 0,
        paidMinutesUsed: userType === 'paid' ? usedMinutes : 0
      };
    }
